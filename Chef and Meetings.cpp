...
int cch(string mt, int t)
{
    int mh;
    mh = 10 *(mt[0 + t] - '0') + 1* (mt[1 + t] - '0');
    if(mt[6+t] == 'P'){if(mh != 12) mh += 12;}
    if(mt[6 + t] == 'A') {if(mh == 12) mh-= 12;}
    return mh;
}
int ccm(string mt, int t)
{
    int mm;
...
int main()
{
    ...
    while(t--)
    {
        ...
        while(n--)
        {
            ...
            int fsh = cch(friendtime, 0);
            int fsm = ccm(friendtime, 0);
            int feh = cch(friendtime, 9);
            int fem = ccm(friendtime, 9);
